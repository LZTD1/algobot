...
type GroupSerializer interface {
	Serialize(group models.Group) (string, error)
}
type MyGroup struct {
	log        *slog.Logger
	grouper    Grouper
...
func (g *MyGroup) ServeContext(ctx telebot.Context) error {
	...
	uid := ctx.Sender().ID
	groups, err := g.grouper.Groups(uid)
	if err != nil {
	...
	return ctx.Send(g.msgMyGroups(groups, ctx), telebot.ModeHTML)
}
...
func (g *MyGroup) getFormattedTitle(group models.Group, ctx telebot.Context) string {
	...
	serialized, err := g.serializer.Serialize(group)
	if err != nil {
