...
func (h *Handler) Handle(ctx context.Context, r slog.Record) error {
	...
	if !levelAttr.Equal(slog.Attr{}) {
		...
		if r.Level <= slog.LevelDebug {
			level = colorize(lightBlue, level) // Debug level
			...
			debugColor = yellow
		} else if r.Level < slog.LevelError { // Warn level
			level = colorize(lightYellow, level) + " "
			debugColor = lightYellow
		} else if r.Level <= slog.LevelError+1 {
			level = colorize(lightRed, level)
			debugColor = lightRed
		} else if r.Level > slog.LevelError+1 {
			level = colorize(lightMagenta, level)
			debugColor = lightMagenta
		}
	}
	...
	if !timeAttr.Equal(slog.Attr{}) {
		timestamp = colorize(lightGreen, timeAttr.Value.String())
	}
	...
	if !msgAttr.Equal(slog.Attr{}) {
		msg = colorize(debugColor, msgAttr.Value.String())
	}
