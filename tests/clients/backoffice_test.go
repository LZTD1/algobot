...
func TestBackoffice(t *testing.T) {
	...
	t.Run("GetGroupInfo", func(t *testing.T) {
		t.Run("200 | Servers return OK", func(t *testing.T) {
			...
			}, "GET", cookie, "", FullGroupInfo)
			defer ts.Close()

			bo := clients.NewBackoffice(ts.URL, boSettings)
			info, err := bo.GetGroupInfo(cookie, "1")
			assertNoError(t, err)

			if info.Data.Title != "Библиотека 10 пн 16.00" {
				t.Fatalf("Wanted: %s, got %s", "Библиотека 10 пн 16.00", info.Data.Title)
			}
			if info.Data.Branch.Email != "moscow@example.org" {
				t.Fatalf("Wanted: %s, got %s", "moscow@example.org", info.Data.Branch.Email)
			}
		})
	})
	t.Run("GetKidInfo", func(t *testing.T) {
		t.Run("200 | Servers return OK", func(t *testing.T) {
			cookie := "111"
			ts := getBOServer(t, map[string]string{
				"expand": "groups",
			}, "GET", cookie, "", FullKidInfo)
			defer ts.Close()

			bo := clients.NewBackoffice(ts.URL, boSettings)
			info, err := bo.GetKidInfo(cookie, "1")
			assertNoError(t, err)

			if info.Data.FullName != "Иван Иванов" {
				t.Fatalf("Wanted: %s, got %s", "Иван Иванов", info.Data.FullName)
			}
			if info.Data.Email != "ivanov-maria@example.com" {
				t.Fatalf("Wanted: %s, got %s", "ivanov-maria@example.com", info.Data.Email)
			}
		})
	})
}
